...
6. Docker registry -- Github (учетка там есть)

7. Описание микросервисов и объектов k8s -- kustomization; не так громоздко как Helm и позволяет держать структуру компактной

8. Namespace -- демонстрация. :) На практике можно прикрутить RBAC, да и удобно

9. ingress -- Traefik; входит в k3s

10. Сертификаты: поддерживается как staging, так и прод, HTTP-01 challenge; свой домен 

11. Мониторинг: стек VictoriaMetrics Single+VictoriaMetrics Alert+Grafana и источники данных/экспортеры метрик node exporter и kube-state-metrics; все с очень урезанными метриками и antiaffinity grafana<->VictoriaMetrics Single. Так удалось разместить все в ресурс

12. GitOps -- flux, опять же из-за ограниченности ресурсов

13. GitOps -- выбор способа получения репозитория (без deploy key/Github PAT)

14. HPA -- выбор метрик для теста. Смог подобрать такие, чтобы и кластер не уронить, и протестировать
